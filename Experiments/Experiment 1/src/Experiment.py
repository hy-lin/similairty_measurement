...
class ExperimentSession(object):
    ...
    def _beginPracticeTrialMessage(self):
        ...
        self.display.clear()
        self.display.drawText(u'Mit Leertaste weiter zu den �bungsaufgaben', font_size=self.exp_parameters.font_size)
    ...
    def _takingBreak(self):
        ...
        self.display.clear()
        self.display.drawText(u'Gelegenheit f�r kurze Pause. Weiter mit Leertaste', font_size=self.exp_parameters.font_size)
        self.display.refresh()
        self.recorder.recordKeyboard(['space'])
        self.display.clear(refresh = True)
    ...
    def _setupStimulusSurface(self):
        ...
        for i in range(self.exp_parameters.n_items):
            ...
            self.stimulus[i].updateStimulusSurface(self.faces_surface)
...
class MultiComparison(ExperimentSession):
    ...
    def _constructPracticeTrials(self):
        ...
        for i in range(self.exp_parameters.n_practice_items):
            self.practice_stimulus_pool.append(Stimulus.ColorPatch(numpy.random.randint(0, 255, size = 3)))
            self.practice_stimulus_pool[i].updateStimulusSurface()
